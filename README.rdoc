...
Tired of remembering every command and subcommand that comes with ruby-debug? This gem solves this
and more, providing ruby-debug command and subcommand completion while providing a completion system
...
In a ruby program gone far far astray:
    ...
    (rdb:1) [TAB]
    backtrace  continue   down       exit       info       next       putl       save       step
    undisplay
    break      delete     edit       finish     irb        p          quit       set        thread
    up
    catch      disable    enable     frame      list       pp         reload     show       tmate
    var
    condition  display    eval       help       method     ps         restart    source     trace
    where
...
This gem doesn't have ruby-debug as a dependency since ruby-debug is split up across two gems
depending on the ruby version. Use whichever ruby-debug gem you need with this gem.
